...
- **`$pageleave` is emitted manually, per route** (the SDK won't do it for us): posthog only fires `$pageleave` from its `_handle_unload` (`pagehide`/`unload`) and only when `Xi()` is true — `capture_pageleave: true`, or the default `"if_capture_pageview"` with `capture_pageview` truthy. Ours is `capture_pageview: false`, so the gate is false and no SDK pageleave ever fires (setting `capture_pageleave: true` would only give one per tab-close, never per SPA route). `TrackPageview` keeps a `{url, startedAt}` ref and, on each pathname change, fires `capturePageLeave(prevUrl, Date.now()-startedAt)` **before** the new `$pageview` — the SDK's pageViewManager then attaches the correct `$pageview_id`/`$prev_pageview_id` (verified: each `$pageleave.$pageview_id` equals its page's `$pageview.$pageview_id`, and pageviews chain via `$prev_pageview_id`). `duration_ms` is our own dwell (the SDK's `$duration` is dead in 1.372 — nothing calls `set_event_timer`). `pagehide` closes the final page with `{transport:"sendBeacon", send_instantly:true}`; `pageshow`/bfcache restore reopens the session.
- Events: `$pageview`, `$pageleave`, `cta_clicked`, `lab_opened`, `lesson_started`, `lesson_completed`, `unit_completed`, `course_completed`, `query_run`, `query_error`, `challenge_started`, `challenge_attempt`, `challenge_completed`, `hint_revealed`, `scenario_run`, `$exception` (via `captureException`). Wiring lives in `AnalyticsProvider` (layout), `TrackedCta` (landing), `LessonView`, `PlanLab`, `ChallengeWorkspace`, `BTreePlayground`, `ConcurrencyPlayground`. All helpers no-op when the key is unset/malformed.
...
- **Capture bodies are gzip-compressed batch POSTs** (and beacon sends are single event **objects**, not arrays — `decodeBody` handles all three shapes). `analytics-probe.mjs` (repo root) decodes via `request.postDataBuffer()` + `gunzipSync` and intercepts `/tt` event POSTs (fulfilled `{"status":1}`, never reaching PostHog); `$pageview` (landing + route changes, `app:"dblearn"`), `cta_clicked` (landing hero — select by `a[href="/learn"]` filter `hasText:"Start lesson one"`, NOT `.first()` which matches the header nav tab), `lesson_started`, per-route `$pageleave` (with matching `$pageview_id` + `duration_ms` + previous URL), the `pagehide` beacon `$pageleave` (asserted by dispatching a synthetic `pagehide`), zero console errors. Run: `node analytics-probe.mjs` (needs prod server on :3000; chromium via `PLAYWRIGHT_CHROMIUM`).
